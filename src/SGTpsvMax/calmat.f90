...
subroutine cala0( nlayer,omega,omegai,t,h1x,h1y,h1z,h2L,h2N, h3ax,h3ay,h3az,h4aL,h4aN, h5ax,h5ay,h5az,h6aL,h6aN, h7x,h7y,h7z,h8L,h8N, coef1,coef2,a0 )
...
subroutine calhml( nlayer,coef1,coef2,h3mx,h3my,h3mz,h5mx,h5my,h5mz,h4m1L,h4m1N,h4m2L,h4m2N,h6m1L,h6m1N,h6m2L,h6m2N,a1 )
  ...
  do j=1,ndc+1
     if ( iphase(j).eq.1 ) then
        ...
        do jj=i1,i2
           ...
           do ii=2,4,2
              a(ii,jj) = a(ii,jj)  +  dcmplx(dble(a0(ii,mtmp)) + l2 *  dble(a2(ii,mtmp)), dimag(a0(ii,mtmp)) + l2 * dimag(a2(ii,mtmp)) )
           enddo
           do ii=1,3,2
...
subroutine calbc( maxnzone,ndc,rdc,iphase,kkdr,a )
  ...
  do i=1,ndc
     if ( (iphase(i).eq.1).and.(iphase(i+1).eq.2) )  a( 2,kkdr(i+1) ) =   dcmplx( rdc(i) * rdc(i) )
     if ( (iphase(i).eq.2).and.(iphase(i+1).eq.1) )  a( 3,kkdr(i+1) ) = - dcmplx( rdc(i) * rdc(i) )
  enddo
...
subroutine calg( l,m,coef1,coef2,lsq,ecC0,ecF0,ecL0,ya,yb,yc,yd,ra,r0,mt,g )
  ...
  if ( iabs(m).eq.0 ) then
     dd = dcmplx( 2.d0 * b1 * mt(1,1) / ( r0 * r0 ) )  / ( dcmplx( ecC0 - 4.d0 / 3.d0 * ecL0 ) * coef2   + dcmplx( 4.d0/3.d0*ecL0 ) * coef1 )
     ee = dcmplx( 0.d0 )
  endif

  if ( iabs(m).eq.0 ) then
     r03 = r0 * r0 * r0
     xtmp =   ( ( ecF0 + 2.d0 / 3.d0 * ecL0) * coef2  - 2.d0 / 3.d0 * ecL0 * coef1 )  / ( (ecC0 - 4.d0 / 3.d0 * ecL0)* coef2   + 4.d0 / 3.d0 * ecL0 * coef1 )
...
subroutine rea2( nn,a,g,c,d, nzone,iphase,kkdr,spn,kkdr0,nn0, maxnstack,nsta,istazone,iista,jsta )
  ...
  do izone=1,nzone
     do ista=1,nsta
        if(izone.eq.istazone(ista)) then
...
subroutine calparderiv(omegar,omegai,l,c,dcdr,z,dzdr,ra, par0,par1,par2)
  ...
  tmp4 = l2 * c(1) * z(1) + ra2 * dcdr(2) * dzdr(2) - ra * ( dcdr(2) * z(2) + c(2) * dzdr(2) ) + c(2) * z(2) - lsq * ( - ra * ( c(1) * dzdr(2) + dcdr(2) * z(1) )   + ( c(1) * z(2) + c(2) * z(1) ) )
